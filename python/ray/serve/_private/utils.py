...
def _register_thread_lock_serializer(serialization_context):
    ...
    FastAPI >= 0.137 embeds a threading.Lock in the ASGI app object, which
    cloudpickle cannot serialize ("cannot pickle '_thread.lock' object").
    serve.ingress(app) pickles the app to freeze it (and again to ship it to
    replicas), so both fail. A lock carries no transferable state and the app is
    frozen/shipped before it serves any request, so reconstruct a fresh,
    unlocked lock on deserialization.
    """
    import threading
